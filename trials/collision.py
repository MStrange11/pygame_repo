...
def bouncing_rect():
    ...
    if moving_rect.colliderect(other_rect):
        ...
        if abs(other_rect.right - moving_rect.left) < collision_tolerance and x_speed < 0:
            x_speed *= -1
        if abs(other_rect.top - moving_rect.bottom) < collision_tolerance and other_speed < 0:
            other_speed *= -1
        if abs(other_rect.bottom - moving_rect.top) < collision_tolerance and other_speed > 0:
            other_speed *= -1

    pygame.draw.rect(screen, (255, 255, 255), moving_rect)
    pygame.draw.rect(screen, (255, 0, 150), other_rect)
